...
# Group random split
def group_random_split(val_frac=1/3, test_frac=7/10*1/3, group_key='group', **kwargs):
    def get(data):
        # inits
        group = [k for k in data[group_key]]
        xval = random_split(test_frac=test_frac,val_frac=val_frac)(np.unique(group))
        for key in xval:
            if len(xval[key][0])>0:
                xval[key] = [np.concatenate([np.where(group == k)[0] for k in xval[key][0]])]
        return xval
    return get

# random split
def random_split(val_frac=1 / 3, test_frac=7/10*1/3, **kwargs):
    def get(data):
